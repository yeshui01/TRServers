/*=======================================================
# Author       : mknight
# Last modified: 2020-02-03 15:57
...
int main(int argc, char* argv[])
{
	...
	std::string log_config = std::string("log_config/view_server_") + std::string(argv[1]) + std::string("_log.properties");
	// log4cplus::Logger logger = log4cplus::Logger::getRoot();
	// log4cplus::PropertyConfigurator::doConfigure(LOG4CPLUS_TEXT(log_config.c_str()));
	g_ServerLog.Init(log_config);
	TDEBUG("hello view server");
	ViewServer view_server(std::stoi(argv[1]));
	view_server.RunService();
	return 0;
}
